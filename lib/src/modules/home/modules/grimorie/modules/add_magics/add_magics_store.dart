...
class AddMagicsStore extends ChangeNotifier {
  ...
  List<Magic> _disabledMagics = [];
  List<Magic> get disabledMagics => _disabledMagics;

  List<Magic> _selectedMagics = [];
  List<Magic> get selectedMagics => _selectedMagics;
  void onSelectMagic(Magic value) {
    if (_selectedMagics.contains(value)) {
      _selectedMagics.remove(value);
    } else {
      _selectedMagics.add(value);
    }
  ...
  List<Magic> get magics => _magics
      .where(
        (m) =>
            (_compareText(m.name) ||
                _compareText(m.desc) ||
                _compareText(m.targetAreaEffect) ||
